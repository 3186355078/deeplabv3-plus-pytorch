...
def init_checkpoint(cfg):
    ...
    if cfg.trainer.resume_dir:
        ...
        if cfg.master:
            logdir = '{}_{}_{}_{}'.format(cfg.trainer.name, cfg.model.name, cfg.data.type,
                                          time.strftime("%Y%m%d-%H%M%S"))
            cfg.logdir = '{}/{}'.format(cfg.trainer.checkpoint, logdir)
            os.makedirs(cfg.logdir, exist_ok=True)
            shutil.copy('{}.py'.format('/'.join(cfg.cfg_path.split('.'))),
...
def accuracy(output, target, topk=(1,)):
    ...
    return [correct[:k].reshape(-1).float().sum(0) * 100. / batch_size for k in topk], [correct[:k].reshape(
        -1).float().sum(0) for k in topk] + [batch_size]
...
class Throughput:
    ...
    @classmethod
    def testall(cls, model, dataloader=None, data_path="", img_size=224, _batch_size=128, with_flops=False,
